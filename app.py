...
def receba():
    ...
    # Decodifico a mensagem com a função decode, qua usa as chaves "p", "q" e "e" de B
    decoded = decode.decode(encoded, key_b_p, key_b_q, key_b_e)
...
def enviar():
    ...
    # Codifico a mensagem com a função decode, qua usa as chaves "p", "q" e "e" de A
    encoded = encode.encode(decoded, key_a_p, key_a_q, key_a_e)
    # Envio ela para o webhook do app2.py
    requests.post("http://localhost:5001/webhook", json={"mensagem":encoded})
    return {"status":"sent"}
